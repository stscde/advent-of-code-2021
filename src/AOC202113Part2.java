...
/**
 * Advent of Code 2021/12/13 Part 2
 */
public class AOC202113Part2 {

	public static void main(String[] args) throws IOException {

		// read input
		BufferedReader br = new BufferedReader(new InputStreamReader(AOC202113Part2.class.getResourceAsStream("AOC202113_input.txt")));
		String line = null;
	...
	private static int[][] foldY(int[][] dots, int f) {
		...
		// copy top part
		int[][] newDots = new int[f][dots[0].length];
		for (int y = 0; y < f; y++) {
		...
		// mirror bottom part
		for (int y = f; y < dots.length; y++) {
			for (int x = 0; x < dots[0].length; x++) {
				int newY = f - (y - f);
				if (dots[y][x] == 1) {
					newDots[newY][x] = dots[y][x];
				}
			}
		}
